...
export function NextRegion({ region, offset }: { region: Region; offset: number }) {
  let regionIdx = regionPageList.findIndex((r) => r.slug === region.slug)
  if (regionIdx < 0) {
    return <div />
  }

  let nextRegionIdx = (regionIdx + regionPageList.length + offset) % regionPageList.length
  let prevRegionIdx = (regionIdx + regionPageList.length - 1) % regionPageList.length
  ...
  return (
    <>
      <Link
        href={urlPartReplacer(paths.regions.region, ":id", regionPageList[nextRegionIdx].slug as string)}
        key={regionPageList[nextRegionIdx].name}
        className="d-flex flex-row align-items-center gap-1"
      >
        {offset < 0 && <ArrowLeftIcon className="text-primary fa-md" />}
        <span className="text-center">{regionPageList[nextRegionIdx].name}</span>
