...
import { todayIso } from "@app/Shared/time"

import * as actionTypes from "./actionTypes"
import { transformPlatformJson } from "./jsonTransformer"
import { ErddapDatasetAndField, ErddapDatasetInfo, PlatformData, PlatformJson } from "./types"
...
export type PlatformDataActions =
  | PlatformDataLoadSuccess
  | PlatformDataLoading
  | PlatformDataError
  | PlatformDataClearError
  | PlatformForecastLoadSuccess
  | PlatformForecastLoading
  | PlatformForecastError
  | PlatformForecastClearError
  | PlatformMetadataLoadSuccess
  | PlatformMetadataLoading
  | PlatformMetadataError
  | PlatformMetadataClearError
 ...
 */
export function platformDataLoadSuccess(
  platformId: string,
  data: PlatformData[]
): PlatformDataLoadSuccess {
  return {
 ...
 */
export const platformDataLoad: ActionCreator<
  ThunkAction<Promise<Action>, StoreState, undefined, Action>
> = (platformId: string) => {
  return async (dispatch: Dispatch): Promise<Action> => {
    try {
      dispatch(platformDataLoading(platformId))
      const url =
        "http://neracoos.org/data/json/buoyrecentdata.php?platform=" +
        platformId +
        "&mp=no&hb=168&tsdt=all"
 ...
 */
export function platformForecastLoadSuccess(
  platformId: string,
  dataset: ErddapDatasetAndField,
  data: PlatformData
): PlatformForecastLoadSuccess {
 ...
 * @returns Formatted Action
 */
export function platformForecastLoading(
  platformId: string,
  dataset: ErddapDatasetAndField
): PlatformForecastLoading {
  return {
 ...
 * @returns Formatted Action
 */
export function platformForecastError(
  platformId: string,
  dataset: ErddapDatasetAndField,
  message: string
): PlatformForecastError {
 ...
 * @returns Formatted Action
 */
export function platformForecastClearError(
  platformId: string,
  dataset: ErddapDatasetAndField
): PlatformForecastClearError {
  return {
 ...
 * @returns Formatted Action
 */
export function platformMetadataSuccess(
  dataset: ErddapDataset,
  coverageStart: Date,
  coverageEnd: Date
): PlatformMetadataLoadSuccess {
 ...
 * @returns Formatted Action
 */
export function platformMetadataError(
  dataset: ErddapDataset,
  message: string
): PlatformMetadataError {
  return {
  ...
  return async (dispatch: Dispatch): Promise<Action> => {
    try {
      ...
      if (coverageStart <= now && now <= coverageEnd) {
        return dispatch(platformMetadataSuccess(dataset, coverageStart, coverageEnd))
      } else {
        // Otherwise notify that there is no data to load.
        return dispatch(
          platformMetadataError(
            dataset,
 ...
 */
export const forecastDataLoad: ActionCreator<
  ThunkAction<Promise<Action>, StoreState, undefined, Action>
> = (
  platformId: string,
  dataset: ErddapDatasetInfo,
  lat: number,
  lon: number,
  datasetAndField: ErddapDatasetAndField
) => {
  return async (dispatch: Dispatch): Promise<Action> => {
    dispatch(platformForecastLoading(platformId, datasetAndField))
    try {
      const url = erddapUrl(
        dataset as ErddapDataset,
        lat,
        lon,
        datasetAndField.field,
        todayIso(),
        dataset.coverageEnd
      )
      const proxyUrl = proxytizeUrl(url)
      ...
      return dispatch(platformForecastLoadSuccess(platformId, datasetAndField, data))
    } catch (error) {
      // tslint:disable-next-line:no-console
      console.log(error)
